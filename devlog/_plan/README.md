...
| 22 | `260515_fork-prompting-modularization-research/` | research / modularization plan | damagethundercat fork와 DCInside 글 기반 Prompt Builder / composer / history UX 모듈화 분석. |
| 23 | `260516_agent-mode-codex-rs-workspace/` | planning | Agent Mode를 Classic/Node 옆 세 번째 workflow로 두되, 선택 시 ChatGPT형 Sessions / Chat / Image workspace로 전환하는 Codex-rs session/compact 기반 UX/런타임 계획. |
...
- 2026-05-15: `260515_fork-prompting-modularization-research/` 추가. `damagethundercat/ima2-gen` fork와 DCInside 글을 Prompt Studio workspace profile, prompt-builder CLI contract, composer/history/viewer 모듈화 관점으로 정리했다. GitHub #59는 아직 open 상태라 active lane으로 유지한다.
- 2026-05-16: `260516_agent-mode-codex-rs-workspace/` 추가. Agent Mode는 Classic composer 내부 패널이 아니라 전체 표면을 Sessions / Chat / Image workspace로 전환하는 세 번째 workflow이며, Codex-rs compact는 대화 기억에만 쓰고 ima2 DB가 이미지 asset/context source of truth를 소유하는 방향으로 정리했다.
